...
class Cluster(object):
    ...
    def fenqun(self):
        ...
        table_name = "./data/task-{}.xlsx".format(1)
        sheet_name = "Sheet{}".format(1)
        data = pd.ExcelFile(table_name)
        task_set = data.parse(sheet_name, index_col="编号")
        task_set = np.array(task_set, dtype='int64')
        ...
        [RR,machine_set,task_set] = cluster.result_return.GA(information_network,task_set,machine_set)
        ...
        Typenum_task = np.zeros([1, np.int(typenum_task)])  # 临时变量，记录当前大任务的前序资源编号
        for i in range(ntask):
