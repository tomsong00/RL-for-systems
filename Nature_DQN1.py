...
class DQN(object):
    ...
    def choose_action_again(self, x, action_choosen):
        # x: a game state
        # 在前面多加一维，可能是一批数据的意思
        # 返回的是0-1动作整数编码
        x = torch.unsqueeze(torch.FloatTensor(x), 0)
        ...
        # input only one sample
        # if np.random.uniform() < EPSILON:   # greedy
        if np.random.uniform() < 60:   # greedy
            actions_value = self.eval_net.forward(x)  # shape=(1,action)
            #想要保存要从cpu转换
            renwushu = np.shape(actions_value)[1]
            if use_gpu:
                actions_value = actions_value.cpu()
                #renwushu = np.shape(actions_value)[0]
            actions_value = actions_value.detach().numpy()
            # print(actions_value)
            # action = np.argmax(actions_value[0])  # 选择回报最大的动作
            # print(action)
            # return action
            actions_value[actions_value <= 0] = 0.001  # 不能有负概率
            tezhengshu = np.shape(actions_value)[0]
            actions_value = np.reshape(actions_value,(-1,renwushu))
            for i in range(renwushu):
                if action_choosen[0][i] == 1:
                    actions_value[:, i] = 0
            # actions_value = actions_value / np.sum(actions_value)  # 归一化
            # 计算排名
            #argsort_ = self.N_ACTIONS - 1 - np.argsort(np.argsort(actions_value[0]))
            ...
            try:
                ...
                action = np.max(actions_value,axis = 0)
                action = np.argmax(action)
                #action = np.random.randint(0, self.N_ACTIONS)
            except:
                #print(actions_value)
...
if __name__ == '__main__':
    ...
    for renwu in range(1):
        for jiqi in range(1):
            task_set = task.load_task(renwu + 1, 1)
            machine_set = machine.load_machine(jiqi + 1, 1)
            ...
            # s = task_set
            # s_ = task_set
            task_size = np.shape(task_set1)[0]
            task_set = reform_task(task_set)
            print('\nCollecting experience...')
            for i_episode in range(0, Epoch):
                # if i_episode%10==0:
                #     print("损失值为{}".format(dqn.se.loss.item()))
                ...
                for t in range(N_ACTIONS):
                    ...
                    if action_choosen[0][a] == 1: #对已经被选择的任务进行处理
                       # 将已经选择过的任务对应的 action_value 变为 0

                       # 再选择一次任务
                       a = dqn.choose_action_again(s, action_choosen)
                       a = int(a)
